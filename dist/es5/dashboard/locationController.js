...
app.controller('LocationController', ['$scope', 'fetcher', '$timeout', '$state', '$stateParams', 'dialog', 'globalVar', function ($scope, fetcher, $timeout, $state, $stateParams, dialog, globalVar) {
	...
	$scope.newLocation = function () {
		dialog.locationDialog({ state: $scope.state, isNew: true, data: { id: defId } }, function (dialResp) {
	...
	$scope.edit = function (o, e) {
		e.stopPropagation();
		dialog.locationDialog({ state: $scope.state, isNew: false, data: _.clone(o) }, function (dialResp) {
			if (_.isObject(dialResp)) {
	...
	$scope.delete = function (locId, question, e) {
		e.stopPropagation();
		dialog.confirm('Are you sure you wanna delete location \"' + question + '\"?', function (response) {
			if (response) {
