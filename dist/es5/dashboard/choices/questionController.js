...
app.controller('ChoicesController', ['$scope', 'fetcher', '$timeout', 'dialog', 'globalVar', function ($scope, fetcher, $timeout, dialog, globalVar) {
	...
	$scope.downloadLink = function () {
		return fetcher.getFilesLink('choicesdata', _.omitBy({
	...
	$scope.changeStatus = function (o, wish, e) {
		e.stopPropagation();

		fetcher.putQuestion(o.ID_question, { status: _.toString(wish) }, function (response) {
			if (response.response == 'OK' && response.status_code == 200) {
				o.status = wish;
			}
		});
	...
	var initData = function initData(like) {
		...
		$scope.nodata = null;
		iterate = 0;
		fetcher.getAllQuestion(_.omitBy({ limit: limit, like: like, offset: 0, category: $scope.category.id, startdate: moment($scope.startDate).format(globalVar.dateFormat), enddate: moment($scope.endDate).format(globalVar.dateFormat) }, _.isNil), function (response) {
			if (response.response == 'OK' && response.status_code == 200) {
