...
app.controller('ChoicesAnsController', ['$scope', 'fetcher', '$timeout', 'dialog', 'globalVar', function ($scope, fetcher, $timeout, dialog, globalVar) {
	...
	$scope.downloadLink = function () {
		return fetcher.getFilesLink('choicesansdata', _.omitBy({
	...
	$scope.loadMoar = function () {
		...
		fetcher.getAllAnswer(data, function (response) {
	...
	$scope.delete = function (id, name, e) {
		...
		dialog.confirm('Are you sure you wanna delete an answer by ' + name + '?', function (response) {
			if (response) {
				fetcher.deleteAnswer(id, function (response) {
	...
	var initData = function initData(like) {
		...
		$scope.nodata = null;
		iterate = 0;
		fetcher.getAllAnswer(_.omitBy({ limit: limit, like: like, offset: 0, category: $scope.category.id, startdate: moment($scope.startDate).format(globalVar.dateFormat), enddate: moment($scope.endDate).format(globalVar.dateFormat) }, _.isNil), function (response) {
			if (response.response == 'OK' && response.status_code == 200) {
