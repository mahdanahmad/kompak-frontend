...
app.factory('dialog', ['ngDialog', function (ngDialog) {
	...
	return {
		...
		adminDialog: function adminDialog(content, callback) {
			var dialog = createDialog(content, 'admin', 'ModalAdminController', 1000);
			dialog.closePromise.then(function (data) {
				callback(data.value);
			});
		},
		profileDialog: function profileDialog(content, callback) {
			var dialog = createDialog(content, 'profile', 'ModalProfileController', 600);
			dialog.closePromise.then(function (data) {
				callback(data.value);
			});
		},
...
app.factory('globalVar', [function () {
	...
	return {
		...
		logsHint: '<ol>' + '<li>Anda dapat mencari logs berdasarkan nama admin dengan mengetik pada kolom yang disediakan.</li>' + '<li>Log dapat difilter berdasarkan aksi dengan menekan kata "aksi".</li>' + '<li>Warna pada latar menunjukkan aksi <span style="color: #949B26;">pengubahan data</span>, <span style="color: #79afb1;">pengisian data</span>, dan <span style="color: #BB342F;">penghapusan data</span>.' + '<ol>'
	};
}]);
